...
CONFIG = {
    # --- 데이터 ---
    "data_dir":     "/home/kim/finetund_retry2",   # 합성 데이터 폴더
    "save_dir":     "./checkpoints_3000",                        # 모델 저장 폴더
    "model_name":   "april_resumed7.pth",                    # 저장할 모델 파일명

    # --- 이어서 학습 (None이면 처음부터) ---                   
    "resume_from":  "best_stage2_unet.pth",                # 기존 pth 파일명 (save_dir 기준)

    # --- 학습 파라미터 ---
    "epochs":       30,          # 이미 수렴 근처 → 길게 할 필요 없음
    "batch_size":   4,
    "lr":           5e-6,        # ★ 핵심: 1e-4 → 5e-6 (20배 낮춤, 기존 가중치 보존)
    "img_size":     1280,
    "patience":     10,          # 더 민감하게 early stop
